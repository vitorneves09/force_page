financeiroReceitaReceberFormController.$inject = ['$scope', '$filter', 'crudServices', 'messageService'];
function financeiroReceitaReceberFormController($scope, $filter, crudServices, messageService) {
    var self = this;
    self.formaPagamentoList = [];
    self.contaList = [];

    $scope.$on('RECEITA_RECEBER_FORM_MODAL', (event, data) => {
        $('#modalReceitaReceberForm').modal('show');
        self.data = {};
        ...
        self.busy = crudServices.getById('financeiro/receita', data.id).then(function (data) {
            self.data = data;
            self.data.juros = 0;
    ...
    self.hide = function () {
        $('#modalReceitaReceberForm').modal('hide');
    }
    ...
    function pagamentoTotal() {
        ...
        messageService.confirm('Confirmar Recebimento', 'Deseja efetuar o recebimento desta conta no valor de\n' + $filter('finance')(self.data.valorPago, true, 2) + '?').then(function (result) {
            if (result.value) {
                self.busy = crudServices.save('financeiro/receita/receber/total', self.data).then(function (data) {
                    self.onUpdate();
                    self.hide();
                    messageService.display('success', 'Conta recebida com sucesso.');
    ...
    function pagamentoParcial() {
        messageService.confirm('Confirmar Recebimento',
            ...
            ' e uma nova divida será criada no valor ' + $filter('finance')(self.data.valorRenegociado, true, 2) + ' com vencimento em ' + $filter('date')(self.data.dataRenegociada, 'dd/MM/yyyy')).then(function (result) {
                if (result.value) {
                    self.busy = crudServices.save('financeiro/receita/receber/parcial', self.data).then(function (data) {
                        self.onUpdate();
                        self.hide();
                        messageService.display('success', 'Conta recebida com sucesso.');
                    }).catch(function (erro) { messageService.display('error', erro.data) });
...
app.component('financeiroReceitaReceberFormComponent', {
    templateUrl: 'app/modules/financeiro/receita/receber/financeiro-receita-receber-form.component.html',
    bindings: {
        onUpdate: '&',
    },
    controller: financeiroReceitaReceberFormController
});