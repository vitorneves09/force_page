financeiroDespesaTituloFormController.$inject = ['$scope', 'moment', 'crudServices', 'messageService']
function financeiroDespesaTituloFormController($scope, moment, crudServices, messageService) {
    var self = this;
    ...
    $scope.$on('DESPESA_TITULO_FORM_MODAL', (event, data) => {
        ...
        self.gruposList[2] = [];
        if (data.id) {
            self.busy = crudServices.getById('financeiro/titulo/despesa', data.financeiroTituloId).then(function (titulo) {
                ...
                titulo.financeiroDespesas = titulo.financeiroDespesas.map(function (item) { item.dataVencimento = moment(item.dataVencimento).startOf('day').toDate(); return item; })
                self.data = titulo;
                self.setGrupos();
    ...
    self.setGrupos = function () {
        self.busy = crudServices.get('financeiro/agrupamento', { where: { tipo: "d" } }).then(function (data) {
    ...
    self.makeParcelas = function () {
        ...
        self.data.financeiroDespesas = retorno;
        self.calcTotalParcelas();
    }
    ...
    self.makeData = function () {
        ...
        for (var i = 0; i < parcelas; i++) {
            var tempData = moment(self.data.dataEntrada).add(i, 'months').startOf('day').toDate();
            self.data.financeiroDespesas[i].dataVencimento = tempData;
        }

        self.data.financeiroDespesas = self.data.financeiroDespesas;
    }

    self.calcTotalParcelas = function () {
        self.totalParcelas = 0;
        self.totalPago = 0;
        self.data.financeiroDespesas.forEach(function (item) {
            self.totalParcelas += item.valorPago || item.valor;
            self.totalPago += item.valorPago;
        })
    }
    ...
    self.save = function () {
        ...
        if (self.totalParcelas.cFloat() != self.data.valor) {
            return messageService.display('warning', 'A soma das parcelas devem ser iguais ao valor do total da despesa.');
        }
    ...
    self.delete = function () {
        messageService.confirm('Confirmar Exclusão', 'Este titulo será excluido.').then(function (result) {
            if (result.value) {
                self.busy = crudServices.remove('financeiro/titulo/despesa', self.data.id).then(function () {
                    self.onUpdate();
                    $('.modal').modal('hide');
    ...
    self.cancelarPagamento = function (parcela) {
        messageService.confirm('Cancelar Pagamento', 'O pagamento da parcela de numero #' + parcela.numero + ' será cancelado.').then(function (result) {
            if (result.value) {
                self.busy = crudServices.save('financeiro/despesa/update/cancelar-pagamento', { id: parcela.id }).then(function () {
                    self.onUpdate();
                    crudServices.getById('financeiro/titulo/despesa', self.data.id).then(function (titulo) {
                        titulo.dataEmissao = moment(titulo.dataEmissao).startOf('day').toDate();
                        titulo.dataEntrada = moment(titulo.dataEntrada).startOf('day').toDate();
                        titulo.financeiroDespesas = titulo.financeiroDespesas.map(function (item) { item.dataVencimento = moment(item.dataVencimento).startOf('day').toDate(); return item; })
                        self.data = titulo;
    ...
    self.showParcelaNumero = function (parcela) {
        ...
        if (parcela.financeiroDespesa) {
            text += '.' + parcela.financeiroDespesa.numero;
        }
        return text;
    }

    function save() {
        self.busy = crudServices.save('financeiro/titulo/despesa', self.data).then(function (data) {
            $('.modal').modal('hide');
            self.onUpdate();
            messageService.display('success', 'Despesa atualizada com sucesso.');
        }).catch(function (erro) { messageService.display('error', erro.data) });
    };

    function add() {
        self.busy = crudServices.add('financeiro/titulo/despesa', self.data).then(function (titulo) {
            $('.modal').modal('hide');
            self.onUpdate();
            messageService.display('success', 'Despesa cadastrada com sucesso.');
        }).catch(function (erro) { messageService.display('error', erro.data) });
    }
...
app.component('financeiroDespesaTituloFormComponent', {
    templateUrl: 'app/modules/financeiro/despesa/titulo/financeiro-despesa-titulo-form.component.html',
    bindings: {
        onUpdate: '&',
    },
    controller: financeiroDespesaTituloFormController
});