clienteDashboardFinanceiroDespesaController.$inject = ['$scope', '$routeParams', 'crudServices', 'moment', 'messageService']
function clienteDashboardFinanceiroDespesaController($scope, $routeParams, crudServices, moment, messageService) {
    var self = this;
    ...
    self.load = function () {
        if (!clienteId) return;
        self.busy = crudServices.get('financeiro/despesa/cliente/' + clienteId, options).then(function (data) {
            self.data = data.rows;
    ...
    self.alterarVencimentoDespesa = function (item) {
        ...
        messageService.confirmInput('Alterar data de vencimento da parcela #' + item.id + '?', 'date').then(function (data) {
            if (data) {
                ...
                self.busy = crudServices.save('financeiro/despesa/update/data-vencimento', update).then(function (data) {
...
app.component('clienteDashboardFinanceiroDespesaComponent', {
    templateUrl: 'app/modules/cliente/dashboard/financeiro/despesa/cliente-dashboard-financeiro-despesa.component.html',
    bindings: {
        onUpdate: '&',
    },
    controller: clienteDashboardFinanceiroDespesaController
});